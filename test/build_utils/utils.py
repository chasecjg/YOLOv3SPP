...
def compute_loss(p, targets, model):  # predictions, targets, model
    ...
    # 通过build_targets去计算所有的正样本
    tcls, tbox, indices, anchors = build_targets(p, targets, model)  # targets
    h = model.hyp  # hyperparameters
    red = 'mean'  # Loss reduction (sum or mean)

    # Define criteria
    # 注意这里函数内部已经使用了更高效的sigmoid
    # pos_weight用来平衡正负样本不均衡的问题，但是这里传递的cls_pw是1，其实没起作用
    # reduction设置最后计算的损失是求和还是求平均
    BCEcls = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device), reduction=red)
    BCEobj = nn.BCEWithLogitsLoss(pos_weight=torch.tensor([h['cls_pw']], device=device), reduction=red)
    ...
    for i, pi in enumerate(p):  # layer index, layer predictions
        # 取出每个预测特征层上的正样本信息，indices就是上面build_targets返回的参数，
        # 是个列表形式，存储上的是每个预测特征层上的构建的正样本，通过索引形式就能获取某一特征层上我们之前匹配或则构建的正样本信息。
        ...
        if nb:
            # 如果存在正样本，就获取当前预测特征层上的所有正样本的信息
            # 对应匹配到正样本的预测信息
            ps = pi[b, a, gj, gi]  # prediction subset corresponding to targets

            # GIoU
            # 计算正对这些正样本预测的xy偏移量和wh的值
            pxy = ps[:, :2].sigmoid()
            # clamp(max=1E3)：设置上限，再乘上他所对应的anchor的宽高
            pwh = ps[:, 2:4].exp().clamp(max=1E3) * anchors[i]
            # 拼接得到预测的box信息
            pbox = torch.cat((pxy, pwh), 1)  # predicted box
            # 计算Giou，pbox是预测的针对正样本的中心点的偏移量，和wh，tbox对应的为gt的信息，这里设置的是GIoU=True表示计算giou，如果是diou=true就计算diou
            giou = bbox_iou(pbox.t(), tbox[i], x1y1x2y2=False, GIoU=True)  # giou(prediction, target)
            # 取均值加到损失函数中
            lbox += (1.0 - giou).mean()  # giou loss

            # Obj
            # 针对正样本，我们对其设置的标签都是为giou的
            tobj[b, a, gj, gi] = (1.0 - model.gr) + model.gr * giou.detach().clamp(0).type(tobj.dtype)  # giou ratio

            # Class
            # 如果目标检测的类别大于1的话，就计算loss—_cls，等于1 的时候不计算
...
def build_targets(p, targets, model):
    ...
    # 遍历每一个预测特征层，记录的是模块的索引，我们的索引对应的就是89,101,113
    for i, j in enumerate(model.yolo_layers):  # j: [89, 101, 113]
        # 获取该yolo predictor对应的anchors
        # 注意anchor_vec是anchors缩放到对应特征层上的尺度
        # 获取对应当前特征层所采用的anchor模板，这里的尺度进行了缩放，缩放到了对应的特征层的尺度
        anchors = model.module.module_list[j].anchor_vec if multi_gpu else model.module_list[j].anchor_vec
        # p[i].shape: [batch_size, 3, grid_h, grid_w, num_params]
        # 获取第i个特政层的shape，转为tensor,去取所索引[3, 2, 3, 2]上的参数，也就是grid_w，grid_h,grid_w，grid_h,
        # 这样gain就变成了 [1,1,grid_w，grid_h,grid_w，grid_h]，后面会让gain跟target相乘，相乘之后就能将每个目标所对应的相对坐标转换成当前特征层上的绝对坐标
        gain[2:] = torch.tensor(p[i].shape)[[3, 2, 3, 2]]  # xyxy gain
        # 获取当前特征层所采用模板的shape，数量3
        na = anchors.shape[0]  # number of anchors
        # [3] -> [3, 1] -> [3, nt]
        # 生成一个序列，从一行三列，变成了三行一列，在赋值nt次，列数变为nt，nt对应target的数量
        at = torch.arange(na).view(na, 1).repeat(1, nt)  # anchor tensor, same as .repeat_interleave(nt)

        # Match targets to anchors
        # targets * gain就是上面说的将target转为当前特征层上的绝对坐标
        a, t, offsets = [], targets * gain, 0
        if nt:  # 如果存在target的话
            # 通过计算anchor模板与所有target的wh_iou来匹配正样本，注意这里anchors是模板，默认三个anchor模板
            # j: [3, nt] , iou_t = 0.20
            # 计算iou(粗略计算),寻找大于给定的位置
            j = wh_iou(anchors, t[:, 4:6]) > model.hyp['iou_t']  # iou(3,n) = wh_iou(anchors(3,2), gwh(n,2))
            # t.repeat(na, 1, 1): [nt, 6] -> [3, nt, 6]
            # 获取正样本对应的anchor模板与target信息，再将target repeat na次通过传入j获取对应的target信息
            a, t = at[j], t.repeat(na, 1, 1)[j]  # filter

        # Define
        # long等于to(torch.int64), 数值向下取整
        # 提取t的前两个元素，转为int类型，转置：image_idx, class
        ...
        # 将当前特征层中的所有正样本的信息append到indices中
        # b对应刚才匹配得到的所有正样本的img的index
        # a对应着所有正样本的对应的anchor模板，0,1,2
        # gj,gi对应每个模板的中心坐标
        # gj.clamp_是将限gi制在特征层的内部，防止越界
        indices.append((b, a, gj.clamp_(0, gain[3]-1), gi.clamp_(0, gain[2]-1)))
        # gxy - gij计算每个正样本和他对应的gt的xy的偏移量，gwh是每个正样本所对应的gt的wh
        tbox.append(torch.cat((gxy - gij, gwh), 1))  # gt box相对anchor的x,y偏移量以及w,h
        # a是针对所有正样本他所对应的anchor模板的索引，得到每个正样本所对应的anchor的宽高
        anch.append(anchors[a])  # anchors
        # c表示针对每个正样本他所匹配对应gt的类别添加到tcls列表中
        tcls.append(c)  # class
