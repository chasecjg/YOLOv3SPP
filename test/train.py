...
def train(hyp):
    ...
    # accumulate:没训练64张图片才更新一次权重，根据batchsize的大小自动调整多少步调整一次参数，可以帮助模型训练
    ...
    # data_dict对应的就是文件夹中的my_data.data文件，train_path对应的是该文件夹下的train对应的路径，打开该文件可以看到里面存放的全部是训练数据集的路径
    ...
    # 定义学习率，就是cos曲线，通过下面注释的代码生成的LR.Png可以看到
...
if __name__ == '__main__':
    ...
    # freeze-layers是否冻结网络部分权重，设置为Ｔrue的话只会训练最后三个预测器，其他的都不会去训练，可以加快训练速度。如果设置为False的话，冻结Ｄarknet53这个网络，其他部分正常训练
